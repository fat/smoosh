var fs = require('fs'),
    ...
    colors = require('../support/colors'),
    uglifyJs = require('../support/UglifyJS'),
    jshint = require('../support/jshint/jshint.js').JSHINT,
    asciimo = require('../support/asciimo').Figlet,
    gzip = require('../support/gzip'),
    rimraf = require('../support/rimraf');
...
var _build = {
    ...
    who.forEach(function (who) {
      for (file in _files[who]) {
        if (who == 'JAVASCRIPT') {
          ...
          that.yui_compress(file, _files[who][file]);
        }
      }
    });
  },
  ...
  yui_compress: function (file, file_contents) {
    _files.CSS_MIN[file] = '';
    if (!fs.stat('./tmp').isDirectory()) {
      console.log('maaaaaaaaaaaaaaaa');
      fs.mkdirSync('./tmp', '0666');
    }
    fs.writeFileSync('tmp/tmp.css', _files.CSS_MIN[file]);
    var cmd = 'java -jar ../yui-compressor-2.4.6.jar --type css ./tmp/tmp.css -o ' +
        this.getBuildPath(file, 'CSS', true)
    exec(cmd, function (err, out) {
      if (err) throw err;
      var f = fs.readFileSync('tmp/tmp.css', 'utf8');
      _files.CSS_MIN[file] += f;
      fs.writeFileSync(this.getBuildPath(file, 'CSS', true), _files.CSS_MIN[file]);
      _write.built('a minified file with yui compressor', file);
    });
    // fs.writeFileSync(this.getBuildPath(file, 'CSS', true), _files.CSS_MIN[file]);
  },
