...
var _interface = {
  ...
  connect: function (config_option, options) {

    var options = options || {}
        , maxAge = options.maxAge || 86400000
        , build = options.build || 'min'
        , env = process.env.NODE_ENV || 'development'
        , compression = env == 'development' ? 'UNCOMPRESSED' : 'COMPRESSED'
        , _served = {
            JAVASCRIPT: {}
          , CSS: {}
        }

    _config.init(config_option);
    ...
    return function (req, res, next) {
      ...
      var who = /text\/css/.exec(req.headers.accept) ? 'CSS' : 'JAVASCRIPT'
        , name = req.url.replace(/^\//, '')
        , maxAge = options.maxAge || 86400000;

      if (keys(_files[who]).indexOf(name) != -1) {
        console.log(who);
        console.log(name);
        fs.readFile(paths[who][compression][name], function(err, buf){
          if (err) return next(err);
          var headers = {
            'Content-Type': 'text/' + who.toLowerCase()
