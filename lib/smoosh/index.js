...
////////  //         //   ///////     ///////   ////////  //      //  \n';
  ...
  jshint: function(key, errors) {
    if (!errors.length) {
      console.log('+' + ' Congratulations. You are the perfect JavaScripter.'.green);
    } else {
      console.log([
        'Boo!'
      , '*' + key + '*',
...
function setup(path_to_config) {
  /*! PROCESS FILES */
  try {
    config = typeof path_to_config == 'string' ?
      JSON.parse(fs.readFileSync(path_to_config, 'UTF-8')) : // 'smoosh.config
